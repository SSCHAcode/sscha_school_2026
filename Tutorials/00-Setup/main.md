...
and the download and extracted data is significative, so it must be performed beforehand.
...
Run and try that the virtual machine works **before** the start of the school.
Note that running the VM for the first time requires a lot of free space on the disc (up to about 30 GB).
...
For that, follow the instructions provided in the official web page: [www.sscha.eu](https://sscha.eu).
